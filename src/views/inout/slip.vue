  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="审核状态" min-width="85" align="center">
        <template slot-scope="scope">
          <el-button v-if="scope.row.audit_status===1" size="mini" type="primary" @click.native.stop="handleAuditSlip(scope.row.id)">
            审核
          </el-button>
      ...
      <el-table-column label="操作" align="center" min-width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            type="info"
            plain
            @click.native.stop="handleGetSlip(scope.row.id)"
          >
            查看
          </el-button>
          <el-button v-if="scope.row.audit_status===1" size="mini" type="danger" @click.native.stop="handleDeleteSlip(scope.row.id)">
            删除
          </el-button>
          <el-button
            size="mini"
            type="success"
            plain
            @click.native.stop="handleDownloadSlip(scope.row.id, scope.row.order_code)"
          >
            导出
          </el-button>
        </template>
...
export default {
  ...
  methods: {
    ...
    handleGetSlip(slip_order_id) {
      this.$router.push({
        name: 'updateSlip',
        params: { inout_id: slip_order_id }
      })
    },

    handleAuditSlip(slip_order_id) {
    ...
    handleDeleteSlip(slip_order_id) {
      this.$confirm('此操作将永久删除该单据, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        const data = { order_id: slip_order_id, order_type: 3 }
    ...
    handleDownloadSlip(inout_id, order_code) {
      this.downloadLoading = true
      downloadSlipExcel({ slip_order_id: inout_id }).then(res => {
