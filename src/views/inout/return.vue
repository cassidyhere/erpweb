<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="退货单编号" prop="order_code" sortable="custom" min-width="140" align="center">
        <template slot-scope="scope">
          {{ scope.row.order_code }}
        </template>
      </el-table-column>
      <el-table-column label="供应商名称" prop="supplier_name" sortable="custom" min-width="150" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.supplier_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="退货日期" sortable="custom" min-width="140" align="center">
        <template slot-scope="scope">
...
import { fetchInoutList, deleteInout, reconcileInout, auditReturn, downloadReturnExcel } from '@/api/inout'
  ...
  methods: {
    ...
    handlereconciled(inout_id) {
      this.$confirm('确认对账?', '提示', {
        ...
        const data = { inout_id: inout_id, order_type: 5 }
